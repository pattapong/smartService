...
namespace SmartService.Printer
{
	public class PrintSlip
	{
		...
		private void LoadConfig(string style,int table_id)
		{
			...
			if ((table_id >= Convert.ToInt32(CheckBillService.GetDescriptionByID("TABLE_SEC_ID"))) && (style != STYLE_KITCHEN))
				this.printerName = CheckBillService.GetDescriptionByID(style + "_2_PRINTER");
			else 
